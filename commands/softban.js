...
module.exports.execute = async (bot, message, content, config, moment, request) => {
    ...
    let notBannableEmbed = new Discord.RichEmbed()
        .setColor(`#${config.colorDanger}`)
        .setDescription(`**<@${message.author.id}>, I can't softban this user! Do they have a higher role?**`);
    ...
    if (content[0]) {
        ...
        try {
            ...
            await message.guild.unban(id).catch(O_o => {});
            let logEmbed = new Discord.RichEmbed()
                .setColor(`#${config.colorInfo}`)
                .setAuthor(`SoftBanned | ${member.user.tag}`, bot.user.displayAvatarURL)
